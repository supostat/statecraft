...
class PaymentsController < ApplicationController
  ...
  # The clean transition: load fresh, fire. A stale form's second submit
  # loads the record already captured, and the event honestly has no branch
  # from there — the staleness family flash, not a silent no-op.
  def capture
    payment = Payment.find(params[:id])
    payment.capture!(metadata: {})
    redirect_to payment_path(payment), notice: "capture fired: the payment is now #{payment[:state]}."
  end
  ...
  def save_and_capture
    ...
    @payment.capture!(metadata: {})
    redirect_to payment_path(@payment), notice: "saved and captured."
